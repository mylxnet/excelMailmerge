# PROJECT\_STATE.md — Excel 格式模板快速生成工具
...
| 模块                   | 文件                                                  | 状态                                                                                                   | <br />                |
| -------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | --------------------- |
| 数据源加载 + 预览           | `Services/DataSourceService.cs` + `MainWindow.xaml` | ✅ 支持选 Sheet、自定义标题行号、DataGrid 动态列                                                                     | <br />                |
| 数据源校验                | `Services/ValidationService.cs`                     | ✅ 检测合并单元格、空标题行、列名重复等                                                                                 | <br />                |
| 模板扫描 + 占位符识别         | `Services/TemplateService.cs`                       | ✅ `{列名}` 正则匹配、高亮显示、匹配状态提示                                                                            | <br />                |
| 试生成预览                | `Services/GenerationEngine.cs::GeneratePreview`     | ✅ 生成后读回填充值展示在 DataGrid                                                                               | <br />                |
| 全部生成                 | `Services/GenerationEngine.cs::GenerateAsync`       | ✅ 多文件/单文件、进度报告、失败重试                                                                                  | <br />                |
| 命名规则                 | `Services/NamingService.cs`                         | ✅ 主命名列 + 次命名列 + 分隔符 + 重名自动追加序号                                                                       | <br />                |
| 自动日期子文件夹             | `GenerationEngine.cs::GenerateAsync`                | ✅ `输出结果yyyyMMdd_HHmmss`                                                                              | <br />                |
| 格式完整保留               | `GenerationEngine.cs::CopySheetContentAndStyle`     | ✅ ClosedXML `Range.CopyTo` 一次性复制值+样式+合并区域                                                            | <br />                |
| 公式转值                 | `GenerationEngine.cs::ConvertFormulasToValues`      | ✅ 扫描所有公式单元格，保留计算值，清空公式表达式                                                                            | <br />                |
| 模板编辑器（外部 Excel/WPS）  | `Views/TemplateEditorWindow.*`                      | ✅ 方案 B：点列名复制剪贴板 + 用 Excel/WPS 打开 + FileSystemWatcher 自动重载                                            | <br />                |
| 四步向导 UI              | `MainWindow.xaml`                                   | ✅ 数据源 → 模板 → 预览 → 生成（渐进解锁）                                                                           | <br />                |
| 文件名预览实时刷新            | `MainViewModel.cs::RefreshFileNamePreview`          | ✅ 单文件显示"合并结果\_xxx.xlsx                                                                               | Sheet:..."、多文件显示每行文件名 |
| 保存文件夹浏览              | `MainViewModel.cs::BrowseFolder`                    | ✅ OpenFileDialog 选文件取路径                                                                              | <br />                |
| self-contained 单文件发布 | csproj + publish 参数                                 | ✅ 免 .NET 运行时，71MB                                                                                    | <br />                |
| WinRAR 自解压安装         | `_installer_stage/setup.bat`                        | ✅ ProgramFiles 安装 + 桌面/开始菜单快捷方式 + 注册表卸载                                                              | <br />                |
| 异常捕获 + 日志            | `App.xaml.cs` + DispatcherUnhandledException        | ✅ 启动崩溃 → `%TEMP%\ExcelMailMerge_startup_error.txt`、运行时崩溃 → `%TEMP%\ExcelMailMerge_runtime_error.txt` | <br />                |
| 命令行测试                | `App.xaml.cs`                                       | ✅ `--gen-test-files` 生成测试数据、`--smoke-test` 端到端冒烟测试                                                   | <br />                |
...
* **现象**：用户选"张三"作为 Sheet 名，实际生成"张三\_信息卡"（"信息卡"是模板 Sheet 名）
...
* **现象**：预览显示用户数据拼接的文件名（"张三\_李四.xlsx"），实际生成"合并结果\_时间戳.xlsx"
...
### BUG-09：行探查循环无限循环 → 每行处理 100+ 秒

* **现象**：RunModeA 单文件模式下，第 1 行就卡了 100 多秒

* **根因**：`CopySheetContentAndStyle` 中行探查循环 `for (r = lastRow+1; r <= lastRow+30; r++)`，循环体内修改了 `lastRow`，边界也跟着变 → 无限循环到 Excel 最大行 1048576；另外 `src.Row(r).Height > 0` 判定自定义行高恒为 true（默认行高 15 > 0）

* **修复**：① 循环边界提前捕获为 `maxRowProbe` 局部变量；② 行高比较改为 `Math.Abs(h - 默认行高) > 0.01`

* **教训**：循环内**绝不要**修改循环边界变量

### BUG-10：GenerateAsync UI 线程阻塞

* **现象**：单文件模式生成时界面假死

* **根因**：`RunModeA`/`RunModeB` 中 ClosedXML 操作全部在 UI 线程上执行（`await Task.Yield()` 在 WPF 中会回到 Dispatcher 线程）

* **修复**：用 `Task.Run(async () => { ... })` 把整个生成逻辑切到线程池

### BUG-11：ProgressBar 不动 + 实时日志不刷新

* **现象**：日志区有内容（ObservableCollection）但进度条不动，后来连日志也不显示

* **根因**：① `GenerationProgress` 是普通类，属性 setter 不触发 PropertyChanged → WPF 不知道 `ProgressPercent`（计算属性）变了；② 之前 `Logs` 是 `List<string>`，WPF 不监听 List 变化

* **修复**：① `GenerationProgress` 实现 `INotifyPropertyChanged`，setter 自动通知；② `Logs` 改为 `ObservableCollection<string>`；③ `OnChanged` 联动通知计算属性 `ProgressPercent` 和 `StatusText`

* **教训**：WPF 绑定的模型类必须实现 `INotifyPropertyChanged`；集合必须用 `ObservableCollection`

### BUG-12：exe 图标导致启动崩溃

* **现象**：加了 `Icon="app.ico"` 后 WPF 启动崩溃

* **根因**：单文件发布时 WPF XAML 的 `Icon` 属性找不到嵌入资源

* **修复**：删掉 XAML 里的 `Icon`，只用 csproj 的 `<ApplicationIcon>app.ico</ApplicationIcon>`（WPF 自动用于窗口标题栏图标）

### BUG-13：多开时新实例杀掉旧实例后自己也退出（进程数变 0）

* **现象**：连续启动 exe 两次，第二次启动后进程数为 0（旧实例被杀死，新实例也退出了）
* **根因**：新实例先 `Kill` 旧实例，再 `new Mutex(true, Name, out createdNew)`。旧进程虽已退出，但其持有的**命名内核对象尚未被系统释放**，`createdNew` 仍返回 false → 新实例走"通知已有实例并退出"分支 → 没有任何实例存活
* **修复**：新增 `TryAcquireMutex(retries, delayMs)`，创建失败时轮询重试（20 次 × 200ms）；`KillRunningInstances` 增加 `WaitForExit(5000)` 确保旧进程彻底退出
* **教训**：依赖命名内核对象（Mutex/Event）判断"是否已存在实例"时，进程被强杀后对象释放存在延迟，必须做重试

### 3.1.1 v1.1.2 启动行为变更

| # | 变更 | 文件 |
| - | ---- | ---- |
| 1 | 启动时自动结束所有残留同类进程（`KillRunningInstances`，含 `Kill(entireProcessTree:true)` + 等待退出） | `App.xaml.cs` |
| 2 | 单实例锁改为重试获取（`TryAcquireMutex`），避免杀死旧实例后自身退出 | `App.xaml.cs` |
...
| #  | 功能                              | 文件                                                              |
| -- | ------------------------------- | --------------------------------------------------------------- |
| 1  | 支持 .xls / .et 文件（自动转换为 .xlsx）   | `Services/FileFormatConverter.cs`（新建）                           |
| 2  | 应用图标 app.ico                    | `app.ico`（新建）                                                   |
| 3  | 单实例运行（Mutex + NamedPipe 激活已有窗口） | `App.xaml.cs` + `MainWindow.xaml.cs`                            |
| 4  | 关闭按钮生成期间禁用                      | `CommonConverters.cs` InverseBoolConverter + `MainWindow.xaml`  |
| 5  | 单文件模式 Sheet 数上限 200 + 弹窗提示      | `Services/ValidationService.cs` + `ViewModels/MainViewModel.cs` |
| 6  | 校验失败不在选择模板时弹窗（只在生成界面提示）         | `MainViewModel.cs::RunValidation`                               |
| 7  | 数据源地址校验失败时清空 + 不记录上次路径          | `MainViewModel.cs::ParseDataSource` + `SettingsService.cs`      |
| 8  | 模板编辑页去掉行号列                      | `Views/TemplateEditorWindow.xaml`                               |
| 9  | 单文件性能优化：模板只打开 1 次               | `Services/GenerationEngine.cs::RunModeA`                        |
| 10 | 列宽复制修复：前几列不跳过 + 向后探查 30 列       | `GenerationEngine.cs::CopySheetContentAndStyle`                 |
| 11 | 按钮顺序对调（浏览在左，刷新在右）               | `MainWindow.xaml`                                               |
| 12 | 实时日志 + 进度条同时刷新                  | `Models/GenerationModels.cs` 实现 INotifyPropertyChanged          |
